...
static int client_config_line_proc(struct mosq_config *cfg, int pub_or_sub, int argc, const char *argv[]);
...
int client_config_load(struct mosq_config *cfg, int pub_or_sub, int argc, const char *argv[])
{
	int rc;
	FILE *fptr;
	char line[1024];
		...
		if(fptr){
			while(fgets(line, 1024, fptr)){
				...
				if(args[1]){
					...
					rc = client_config_line_proc(cfg, pub_or_sub, count, (const char**)args);
					if(rc){
						fclose(fptr);
						free(loc);
						return rc;
...
int client_config_line_proc(struct mosq_config *cfg, int pub_or_sub, int argc, const char *argv[])
{
	int i;
